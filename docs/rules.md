...
- 기본 한글 폰트는 **`Noto Sans KR`**. 폰트는 Figma 클라우드 보유분만 쓸 수 있으며 **로컬 다운로드는 무의미**합니다. 사용 가능 목록과 권장 조합 → [fonts.md](./fonts.md)
...
| # | 항목 | 왜 필요한가 |
|---|---|---|
| 2 | 디자인 시스템 입력 형식 | 폰트/색상/키워드를 어떤 파일 형태로 받을지 미정 — baseline은 기획안 컨셉 sheet + 코퍼스 팔레트에서 도출로 대체 |
| 3 | 파일 분할 단위 | 제품 1개 = 파일 1개인지, 여러 제품을 한 파일에 페이지로 나눌지 미정 |
| 4 | 간편블럭국 "수상부분 겹침" 블록 특정 | CHECK 연작(`1:85228` `1:85220` `1:84912`) 중 하나로 추정, 미확정 |
...
- ~~한글 폰트 검증~~ → 10종 실측 확인, 기본값 `Noto Sans KR` → [fonts.md](./fonts.md)
- ~~`Section 4` 제품명~~ → **아기 이불(유아 침구)** — "FOR MY BABY" 훅 실측 (`1:85855`)
- ~~토마토 코멘트 2개 중복~~ → `10:86250`("포근한 브라운, 구매후기 전문 지적")은 내용상 **아기 이불의 코멘트**로 판정. 아기 이불 리뷰 블록 `1:86047`(전문 나열)과 정확히 부합
- ~~Pretendard 사용 가능 여부~~ → **로드 불가** 확정, 치환 정책 수립 → [fonts.md](./fonts.md)
- ~~기획안 xlsx 실제 양식~~ → **확보** (2026-08-09, 디자인23 구스밀 v6). 파서·양식 문서 → [pipeline/README.md](../pipeline/README.md), 파싱 결과 `data/briefs/`
