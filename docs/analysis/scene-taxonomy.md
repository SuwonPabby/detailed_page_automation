...
## 신규 장면 후보 (2026-08-15 외부 리서치 — research-detail-page.md §6)

코퍼스 재실측 후 정식 편입 여부를 결정한다. 후보 4종:

| 후보 | 근거 | 비고 |
|---|---|---|
| `motion-hook` | 상단 5–10초 무음 세로 영상/GIF가 2026 훅 표준 (시선 체류 2.6초 대응) | hook의 변형. 쿠팡은 GIF 불가 → JPG 연속컷 |
| `fulfillment-trust` | 신선식품은 배송·콜드체인·검수가 신뢰 블록의 일부 | 기존 trust-cert와 분리 |
| `evidence-strip` | 판매량·평점 요약 배지 스트립을 훅 직후(블록 2~4)에 — 사회적 증거 상단 배치가 2026 표준 | review의 압축 변형. **하단 단일 review는 anti-pattern** |
| `brand-story` 위치 규칙 | 창립 서사는 중후반 이완(D→L) 지점 | 기존 유형의 배치 규칙 추가 |
